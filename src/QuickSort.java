...
public class QuickSort<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> T[] sort(PIVOT_TYPE pivotType, T[] unsorted) {
        ...
        if (pivotType == PIVOT_TYPE.MIDDLE) {
            ...
            pivot = getRandom(unsorted.length);  //Debugged + 1
        }
    ...
    private static <T extends Comparable<T>> void sort(int index, int start, int finish, T[] unsorted) {
        if (start >= finish) //Added if statement to return if array is empty
        {
            return;
        }
        ...
        int f = finish;
        while (s <= f) {
            while (s <= finish && unsorted[s].compareTo(pivot) < 0)
                s++;
            while (f >= start && unsorted[f].compareTo(pivot) > 0) //Debugged
                f--;
            if (s <= f) {
                swap(s, f, unsorted);
                s++;
